"""
参考：https://github.com/stober/gmm
...
class Guass_distribution:
	...
	def init(self,method='random',data=None):
		"""
		高斯分布有2种初始化方法：
	...
	def init_mu(self,range_=(-5,5)):
		self.mu = np.random.randint(*range_,size=(self.dim,1))
		self.mu = np.array(self.mu)

	def init_Sigma(self,sqrt_max_=2):
		"""
		协方差矩阵是对称的半正定阵。
		一个可逆方阵与它转置的內积一定是正定阵，因此这样初始化协方差矩阵。
		"""
		C = None
		while C is None:
			C = np.random.random(size=(self.dim,self.dim))*sqrt_max_
			try:
				np.mat(C).I
			except np.linalg.linalg.LinAlgError:
				C = None
		self.Sigma = C.dot(C.T)
...
class GMM:
	...
	def _init_parameters(self,X,method='kmeans'):
		...
		for i in range(self.m):
			guass = self.Guass[i]
			data = X[clusters[i]]
			guass.init(method='by_data',data=data)
...
class CLF:
	...
	def fit(self,X,Y):
		...
		for i in range(self.k):
			...
			data = X[data_index]
			self.gmms[i].fit(X,max_iter=200)
	...
	def predict(self,X):
		...
		for i in range(k):
			Prob[:,i] = np.log(gmms[i].cal_prob(X))+np.log(self.cluster_prob.reshape((1,self.k))) # (n,k) + (1,k)
